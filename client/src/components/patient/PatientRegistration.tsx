...
const PatientRegistration: React.FC<PatientRegistrationProps> = ({ 
    ...
    try {
      const response = await fetch('http://localhost:5000/api/v1/patients', {
        ...
        body: JSON.stringify({
          ...
          // Add other fields as needed, e.g. contactInfo, location, etc.
          userId: "68346dd7a1c6f81ab85b448f" // Real userId from your database
        })
      });
      let createdPatient;
      ...
      if (!response.ok) {
        throw new Error(createdPatient.message || 'Failed to register patient');
      }
      onRegistrationComplete(createdPatient);
      toast({
        title: "Patient registered",
        description: `${newPatient.name} has been successfully registered.`
      });
    } catch (error: unknown) {
      const errMsg = error instanceof Error ? error.message : 'Could not register patient';
      toast({
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto border-border shadow-lg sm:rounded-lg sm:p-8 p-4">
      <CardHeader>
